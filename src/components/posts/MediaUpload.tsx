...
export const MediaUpload = ({ onFilesSelected, previewUrls, onDeleteImage }: MediaUploadProps) => {
  const onDrop = useCallback((acceptedFiles: File[]) => {
    const remainingSlots = 4 - previewUrls.length;
    if (remainingSlots > 0) {
      onFilesSelected(acceptedFiles.slice(0, remainingSlots));
    }
  }, [onFilesSelected, previewUrls.length]);
      ...
      {previewUrls.length < 4 && (
        ...
        >
          ...
          <p className="text-sm text-muted-foreground">
            Supports: PNG, JPG, GIF (Max {4 - previewUrls.length} more {previewUrls.length === 3 ? 'image' : 'images'})
          </p>
        </div>
      )}
