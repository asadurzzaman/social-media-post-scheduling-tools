...
export const InstagramAuthHandler = () => {
  ...
  const handleInstagramAuth = async () => {
    try {
      ...
      const { instagram_app_id } = credentials;
      ...
      const instagramAuthUrl = `https://api.instagram.com/oauth/authorize?client_id=${instagram_app_id}&redirect_uri=${encodeURIComponent(redirectUri)}&scope=instagram_basic,instagram_content_publish&response_type=code&state=${state}`;
      ...
      window.addEventListener('message', async (event) => {
        if (event.data.type === 'instagram_auth') {
          ...
          const { accessToken, userId, username } = authData;
